...
echo [4/4] Building Tauri shell (release profile, default icon)...
...
echo.
echo DONE. Release binary: apps\shell\src-tauri\target\release\pi-desktop-shell.exe
echo Note: the release shell serves the built UI from apps\ui\dist (no dev server needed),
echo and bundling/installers are disabled in tauri.conf.json (bundle.active=false).
exit /b 0
